const ArgumentType = require('../../extension-support/argument-type');
const BlockType = require('../../extension-support/block-type');
const formatMessage = require('format-message');
// const MathUtil = require('../../util/math-util');

/**
 * Icon svg to be displayed at the left edge of each extension block, encoded as a data URI.
 * @type {string}
 */
// eslint-disable-next-line max-len
// 这是一支笔
// const blockIconURI = 'data:image/svg+xml;base64,';
// 微信聊天框
const blockIconURI = 'data:image/png;base64,';
const menuIconURI = blockIconURI; 

class Scratch3AutopilotAutoBlocks {
    constructor (runtime) {
        /**
         * The runtime instantiating this block package.
         * @type {Runtime}
         */
        this.runtime = runtime;
    }


    /**
     * The key to load & store a target's pen-related state.
     * @type {string}
     */
    static get STATE_KEY () {
        return 'Scratch.autopilotauto';
    }

    /**
     * @returns {object} metadata for this extension and its blocks.
     */
    getInfo () {
        return {
            id: 'autopilotauto',
            name: formatMessage({
                id: 'autopilotauto.categoryName',
                default: '自动模式',
                description: 'Label for the auto pilot auto extension category'
            }),
            // menuIconURI: menuIconURI,
            blockIconURI: blockIconURI,
            // showStatusButton: true,
            blocks: [
                // {
                //     opcode: 'say',
                //     blockType: BlockType.COMMAND,
                //     text: formatMessage({
                //         id: 'autopilotauto.say',
                //         default: 'say [TEXT]',
                //         description: 'say something'
                //     }),
                //     arguments: {
                //         TEXT: {
                //             type: ArgumentType.STRING,
                //             defaultValue: formatMessage({
                //                 id: 'autopilotauto.defaultTextToSay',
                //                 default: 'this is an autopilotauto extension',
                //                 description: 'default text to say.'
                //             })
                //         }
                //     }
                // },
                {
                    opcode: 'setCarName',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.setCarName',
                        default: "设置小车名字为[NAME]",
                        description: 'set the autopilot car name to a particular value'
                    }),
                    arguments: {
                        NAME: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfCar',
                                default: 'Car_0',
                                descriptin: 'default name of car'
                            })
                        }
                    }
                },
                {
                    opcode: 'setClock',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.setClock',
                        default: "为[CLOCK]安装时钟",
                        description: 'set clock to a particular value'
                    }),
                    arguments: {
                        CLOCK: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfClock',
                                default: 'Clock_0',
                                descriptin: 'default clock of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'setCamera',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.setCamera',
                        default: "为[CAMERA]安装摄像头",
                        description: 'set camera to a particular value'
                    }),
                    arguments: {
                        CAMERA: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfCamera',
                                default: 'Camera_0',
                                descriptin: 'default camera of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'setIntelligentBrain',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.setIntelligentBrain',
                        default: "为[BRAIN]安装智能脑",
                        description: 'set brain to a particular value'
                    }),
                    arguments: {
                        BRAIN: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfBrain',
                                default: 'Brain_0',
                                descriptin: 'default brain of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'setNeuralNetworkDecoder',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.setNeuralNetworkDecoder',
                        default: "为[DECODER]安装神经网络解码器",
                        description: 'set decoder to a particular value'
                    }),
                    arguments: {
                        DECODER: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfDecoder',
                                default: 'Decoder_0',
                                descriptin: 'default decoder of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'addDynamicSystem',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.addDynamicSystem',
                        default: "为[DYNAMIC]添加动力系统",
                        description: 'add dynamic system to a particular value'
                    }),
                    arguments: {
                        DYNAMIC: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfDynamic',
                                default: 'Dynamic_0',
                                descriptin: 'default dynamic system of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'addSteeringSystem',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.addSteeringSystem',
                        default: "为[STEER]添加转向系统",
                        description: 'add steering system to a particular value'
                    }),
                    arguments: {
                        STEER: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfSteer',
                                default: 'Steer_0',
                                descriptin: 'default steering system of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'addPictureWarehouse',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.addPictureWarehouse',
                        default: "为[PICTURE]添加图片仓库",
                        description: 'add Picture Warehouse to a particular value'
                    }),
                    arguments: {
                        PICTURE: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfPicture',
                                default: 'Picture_0',
                                descriptin: 'default Picture Warehouse of something'
                            })
                        }
                    }
                },
                {
                    opcode: 'startUp',
                    blockType: BlockType.COMMAND,
                    text: formatMessage({
                        id: 'autopilotauto.startUp',
                        default: "启动[START]",
                        description: 'start up sth'
                    }),
                    arguments: {
                        START: {
                            type: ArgumentType.STRING,
                            defaultValue: formatMessage({
                                id: 'autopilotauto.defaultTextOfStart',
                                default: 'Start_0',
                                descriptin: 'the name of sth to start up'
                            })
                        }
                    }
                },
            ],
            menus: {}
        };
    }

    // say (args, util) {
    //     const message = args.TEXT;
    //     console.log(message);
    //     this.runtime.emit('SAY', util.target, 'say', message);
    // }
    setCarName(args, util){
        const name = args.NAME;
        console.log(name);
    }
    setClock(args, util){
        const clock_value = args.CLOCK;
        console.log(clock_value);
    }
    setCamera(args, util){
        const camera_value = args.CAMERA;
        console.log(camera_value);
    }
    setIntelligentBrain(args, util){
        const ib_value = args.BRAIN;
        console.log(ib_value);
    }
    setNeuralNetworkDecoder(args, util){
        const nnd_value = args.DECODER;
        console.log(nnd_value);
    }
    addDynamicSystem(args, util){
        const ds_value = args.DYNAMIC;
        console.log(ds_value);
    }
    addSteeringSystem(args, util){
        const ss_value = args.STEER;
        console.log(ss_value);
    }
    addPictureWarehouse(args, util){
        const pw_value = args.PICTURE;
        console.log(pw_value);
    }
    startUp(args, util){
        const su_value = args.START;
        console.log(su_value);
    }
}

module.exports = Scratch3AutopilotAutoBlocks;